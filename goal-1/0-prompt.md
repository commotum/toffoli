...
```text
Work through `goal-1/0-plan.md` using the execution protocol in `goal-1/0-loop.md`.
...
For each session: sync actual files and tests; update current facts in the plan; select the first incomplete stage; create or refresh its stage file from the loop template; identify owner modules, high-fanout files to avoid, and focused build targets; implement only that stage; add no-cheating verification; run focused leaf and necessary adjacent-consumer checks, using full builds only for `BUILD-PLAN.md` triggers or milestones; run diff/whitespace checks; record evidence and material build timing in the stage file; and fold the results back into the plan. If stopping, leave exact evidence, next experiments, unblock actions, and assumptions to challenge.
